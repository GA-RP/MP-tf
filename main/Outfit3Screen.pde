//class Outfit3Screen que implementa a interface Screen
class Outfit3Screen implements Screen {

  Movie video;
  boolean finished = false;

  Outfit3Screen(Movie video) {
    this.video = video;
    video.play();
  }
  ...
  public void update() {
    ...
    if (!finished && video.duration() > 0 && video.time() >= video.duration() - 0.05) {
      ...
      currentScreen = new WakeUpChoiceScreen(frames[3]);
    }
  }
