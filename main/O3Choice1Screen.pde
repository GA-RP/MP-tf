//class O3Choice1Screen que implementa a interface Screen
class O3Choice1Screen implements Screen {

  PImage bg;  //variável que guarda o frame
  Button[] buttons = new Button[2];  //array que guarda os 2 botões deste ecrã

  O3Choice1Screen(PImage frame) {  //construtor, recebe o frame
    ...
    buttons[0] = new Button(bx - spacing / 2, by, bw, bh, "Deixar Passar");  // botão 1
  ...
  public void handleMousePressed() {
    ...
    if (buttons[0].isClicked()) {
      currentScreen = new VideosScreen(videos[7], new O3Choice2Screen(frames[4]));  // opção 1
    }

    //se clicar no botão 2
    if (buttons[1].isClicked()) {
      currentScreen = new VideosScreen(videos[8], new O3Choice2Screen(frames[4]));  // opção 2
    }
  }
} //c
