//class O1Choice2Screen que implementa a interface Screen
class O1Choice2Screen implements Screen {

  PImage bg;  //variável que guarda o frame
  Button[] buttons = new Button[2];  //array que guarda os 2 botões deste ecrã

  O1Choice2Screen(PImage frame) {  //construtor, recebe o frame
  ...
  public void handleMousePressed() {
    ...
    if (buttons[0].isClicked()) {
      currentScreen = new VideosScreen(videos[4], new VideosScreen(videos[12], new Creditos()));  // opção 1
    }

    //se clicar no botão 2
    if (buttons[1].isClicked()) {
      currentScreen = new VideosScreen(videos[5], new VideosScreen(videos[13], new Creditos()));  // opção 2
    }
  }
} //c
