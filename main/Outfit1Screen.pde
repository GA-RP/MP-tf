...
class Outfit1Screen implements Screen {
  ...
  Outfit1Screen(Movie video) {  //construtor recebe o vídeo a reproduzir
  ...
  public void update() {
    ...
    if (!finished && video.duration() > 0 && video.time() >= video.duration() - 0.05) {
      ...
      currentScreen = new WakeUpChoiceScreen(frames[1]);  //usa o frame correspondente carregado na main
    }
  }
